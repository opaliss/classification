...
def open_data(name, mode='r'):
    """open_data(name, mode)
    Open a data set that is in the data subdirectory of this package
    """

    # Derive package path from module name
    aima_root = os.path.dirname(__file__)
    # Construct new name
    aima_file = os.path.join(aima_root, *['aima-data', name])

    return open(aima_file, mode=mode)  # Return handle to open file
...
def unique(seq):
    ...
    s = set(seq)  # Only keep unique values
    return list(s)  # return as list
...
def fix_classes(dataset):
    "fix_classes - Ensure numerical target classes are numbered 0:N-1"

    # Get sorted list of values
    values = dataset.values[dataset.target]
    if not isinstance(values[0], str) and \
            (values[0] != 0 or values[-1] != len(values) - 1):
        # Not 0:N-1, build dictionary and renumber
        rename = dict()
        ...
        for idx in range(len(dataset.examples)):
            dataset.examples[idx][dataset.target] = \
                rename[dataset.examples[idx][dataset.target]]
